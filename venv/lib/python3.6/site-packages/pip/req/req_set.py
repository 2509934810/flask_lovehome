...
from pip.download import is_file_url, is_dir_url, is_vcs_url, url_to_path, unpack_url
...
from pip.utils import display_path, dist_in_usersite, ensure_dir, normalize_path
...
class IsWheel(DistAbstraction):
    def dist(self, finder):
        return list(pkg_resources.find_distributions(self.req_to_install.source_dir))[0]
...
class RequirementSet(object):
    def __init__(
        ...
        if session is None:
            raise TypeError(
                "RequirementSet() missing 1 required keyword argument: " "'session'"
            )
    ...
    def add_requirement(self, install_req, parent_req_name=None, extras_requested=None):
        ...
        if not name:
            ...
            if (
                ...
                and not existing_req.req.specifier == install_req.req.specifier
            ):
                raise InstallationError(
            ...
            if not existing_req:
                ...
                if not install_req.constraint and existing_req.constraint:
                    ...
                    existing_req.extras = tuple(
                        sorted(set(existing_req.extras).union(set(install_req.extras)))
                    )
                    logger.debug(
    ...
    def prepare_files(self, finder):
        ...
        require_hashes = self.require_hashes or any(
    ...
    def _is_upgrade_allowed(self, req):
        return self.upgrade and (
            self.upgrade_strategy == "eager"
            or (self.upgrade_strategy == "only-if-needed" and req.is_direct)
        )
        ...
        if req_to_install.satisfied_by:
            ...
            if upgrade_allowed:
                ...
                if not best_installed:
                    ...
                    if not (
                        self.use_user_site
                        and not dist_in_usersite(req_to_install.satisfied_by)
                    ):
    ...
    def _prepare_file(
        self, finder, req_to_install, require_hashes=False, ignore_dependencies=False
    ):
        ...
        with indent_log():
            ...
            if req_to_install.editable:
                ...
                req_to_install.populate_link(
                    finder, self._is_upgrade_allowed(req_to_install), require_hashes
                )
                ...
                if require_hashes:
                    ...
                    if (
                        not req_to_install.original_link
                        ...
                        raise HashUnpinned()
                hashes = req_to_install.hashes(trust_internet=not require_hashes)
