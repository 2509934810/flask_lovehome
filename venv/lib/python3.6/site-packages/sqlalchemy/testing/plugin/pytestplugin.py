            ...
            arg_sets = [
                pytest.param(
                    *_arg_getter(_filter_exclusions(arg))[1:],
                    id="-".join(comb_fn(getter(arg)) for getter, comb_fn in fns)
                )
                for arg in [
                    (arg,) if not isinstance(arg, tuple) else arg for arg in arg_sets
                ]
            ]
        else:
            ...
            if inspect.isclass(fn):
                ...
                if exclusion_combinations:
                    for exclusion, combination in exclusion_combinations:
                        combination_by_kw = {
                            argname: val for argname, val in zip(_argnames, combination)
                        }
                        exclusion = exclusion.with_combination(**combination_by_kw)
                        fn = exclusion(fn)
